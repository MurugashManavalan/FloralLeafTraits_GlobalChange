...
anova(sq_SSAmodel)
anova_SSA <- as.data.frame(anova(sq_SSAmodel))
write_csv(anova_SSA, file = "anova_SSA.csv")
...
View(AusRDL)
rda.l <- rda(AusRDL[,c(17:21)] ~ `CO2 Level`*`Temperature Level`*Drought, data = AusRDL, scale = TRUE)
...
anova.cca(rda.l, permutations = 9999, by = "terms")
anova.cca(rda.ln, permutations = 9999, by = "terms")
View(AusRD)

## Redundancy Analysis for Crepis ----
AusRDC <- AusRD[AusRD$Species == "Crepis",]
rda.c <- rda(AusRDC[,c(17:22)] ~ `CO2 Level`*`Temperature Level`*Drought, data = AusRDC, scale = TRUE)
ordiplot(rda.c,type = "text", scaling = 2)
...
rda.1 <- rda(AusRD[,c(17:21)] ~ `CO2 Level`*`Temperature Level`*Drought, data = AusRD, scale = TRUE)
