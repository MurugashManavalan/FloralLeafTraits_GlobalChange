...
ggplot(AusL, aes(x = CO2, y = `Display Area (DA)(cm2)`, fill = Temperature)) +
  ...
  labs(
    title = "Lotus - Effect of CO2 Level on DA by Temperature Level",
    x = "CO2 Level",
    y = "Display Area (DA)(cm2)",
...
ggplot(AusC, aes(x = AusC$Drought, y= `Average Ray Floret Area (ARA)(cm2)`)) +
  geom_boxplot()+
  labs(
    title = "Crepis - Effect of Drought on ARA",
    x = "Drought Level",
    y = "Average Ray Floret Area (ARA)(cm2)"
  )
...
names(Crecor_Data)
names(Crecor_Data)[names(Crecor_Data) == "Average Ray Floret Area (ARA)(cm2)"] <- "ARA"
